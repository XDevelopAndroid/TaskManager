...
import uz.xdevelop.todo_uz.ui.fragments.TodoFragment.Companion.context
...
class TodoFragmentAdapter(val forSearch: Boolean = false) :
    ...
    private var listenerTag: SingleBlock<String>? = null
    ...
    fun update(person: TodoData) {
        for (i in 0 until sortedList.size()) {
            if (sortedList.get(i).id == person.id) {
                ...
                if (person.todo_deleted) {
                    sortedList.removeItemAt(i)
                    showMessage("Vazifa o'chirilgan vazifalar bo'limiga o'tkazildi!")
                    return
                }
    ...
    fun setOnItemTagListener(block: SingleBlock<String>) {
        listenerTag = block
    }
                    ...
                    when (item?.itemId) {
                        R.id.popup_menu_delete -> {
                            ...
                            dialog.setPositiveButton("Ha") { _, _ ->
                                sortedList[pos].todo_deleted = true
        ...
        fun bind() = bindItem {
            ...
            if (d.tags != null) {
                ...
                d.tags!!.forEach { str ->
                    ...
                    chip.textSize = 12f
                    chip.setOnClickListener { listenerTag?.invoke(str) }
